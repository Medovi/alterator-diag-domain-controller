...
%changelog
* Thu Oct 10 2024 Sergey Savelev <savelevsa@basealt.ru> 0.0.2-alt2
...
- Added a verification test: for whether the search for the kerberos domain name via DNS is enabled.
- Added a test to check for the presence of the resolv.conf file.
- Added a test to check the match of the realm record from the krb5.conf file and the domain name from the resolv.conf file.
- Added a test to check the match of the realm record from the smb.conf file and from the krb5.conf file.
